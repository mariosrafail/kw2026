...
func run() -> void:
	...
	var failure_count:=0
	stage.weapon_side=-1.0;stage.smoothed_weapon_side=-1.0
	var q:=InputEventKey.new();q.physical_keycode=KEY_Q;q.pressed=true
	stage._unhandled_input(q)
	if stage.weapon_side!=1.0 or stage.smoothed_weapon_side!=1.0:
		failure_count+=1;print("RIGHT_HOLD_Q_FAIL")
	for yaw in [0.0,1.4,-2.4]:
		...
		stage._update_weapon_pose(0.25)
		if stage.weapon_side!=1.0 or absf(stage.smoothed_weapon_side-1.0)>0.001:
			failure_count+=1;print("RIGHT_HOLD_DRIFT ",yaw)
		...
		if desired.dot(stage.weapon_root.global_basis.x.normalized())<0.99998:
			failure_count+=1;print("AIM_FAIL ",yaw)
		for gun in stage.weapon_root.get_children():
			...
			for torso in stage.torso_rig.get_children():
				...
				if overlaps(gun,torso):
					failure_count+=1
					if failure_count<6:print("CLIP ",yaw," ",gun.name," ",torso.name)
	await process_frame
	...
	if center.distance_to(crosshair.get_global_rect().get_center())>0.01:
		failure_count+=1;print("RETICLE_FAIL ",center)
	stage.queue_free()
	for i in range(8):await process_frame
	print("RIGHT_SHOULDER_STABILITY_RETICLE_","PASS" if failure_count==0 else "FAIL"," failures=",failure_count)
