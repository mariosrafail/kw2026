...
func run() -> void:
	...
	check(stage.ammo_in_mag==1 and "KAR" in stage.player_ammo_label.text,"kar_slot_has_one_round")
	stage.shot_cooldown=0.0;stage._fire_physics_ball()
	check(stage.ammo_in_mag==0 and stage.reload_remaining>1.30,"kar_single_round_auto_reload")
	for i in range(86):stage._tick_reload(1.0/60.0)
	check(stage.ammo_in_mag==1 and stage.reload_remaining<=0.0001,"kar_reload_refills_one")
