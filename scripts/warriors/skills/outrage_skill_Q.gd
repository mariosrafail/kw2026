...
const BOMB_TEXTURE := preload("res://assets/textures/effects/outrage_bomb.png")
...
func _execute_client_visual(_caster_peer_id: int, target_world: Vector2) -> void:
	client_spawn_bomb(_caster_peer_id, target_world, FUSE_SEC)
...
func client_spawn_bomb(caster_peer_id: int, world_position: Vector2, fuse_sec: float) -> void:
	...
	var bomb_color := _outrage_bomb_color_for_peer(caster_peer_id)

	var bomb_node := Node2D.new()
	bomb_node.z_index = 42
	bomb_node.global_position = world_position
	bomb_node.set_meta("bomb_color", bomb_color)
	...
	bomb_sprite.modulate = bomb_color
...
func _schedule_end_particles(bomb_node: Node2D, fuse_sec: float) -> void:
	...
	timer.timeout.connect(func() -> void:
		...
		var bomb_color_value: Variant = resolved_bomb.get_meta("bomb_color", BOMB_TINT)
		var bomb_color := bomb_color_value as Color if bomb_color_value is Color else BOMB_TINT
		_spawn_end_particles(resolved_bomb.global_position, bomb_color)
	)

func _spawn_end_particles(world_position: Vector2, bomb_color: Color = BOMB_TINT) -> void:
	if projectile_system == null or projectile_system.projectiles_root == null:
		return
	var visual_root: Node2D = projectile_system.projectiles_root
	...
	var burst_color := _outrage_bomb_particle_color(bomb_color)
...
func _outrage_bomb_color_for_peer(caster_peer_id: int) -> Color:
	var player = players.get(caster_peer_id, null)
	if player != null and player.has_method("get_torso_dominant_color"):
		...
		if color_value is Color:
			return _normalize_outrage_bomb_color(color_value as Color)
	return BOMB_TINT

func _normalize_outrage_bomb_color(base_color: Color) -> Color:
	var saturated := base_color.lerp(Color(1.0, 0.42, 0.08, 1.0), 0.18)
	var energy := maxf(saturated.r, maxf(saturated.g, saturated.b))
	if energy < 0.35:
		saturated = saturated.lerp(BOMB_TINT, 0.35)
	return Color(
		clampf(saturated.r * 1.1, 0.0, 1.0),
		clampf(saturated.g * 1.1, 0.0, 1.0),
		clampf(saturated.b * 1.1, 0.0, 1.0),
		1.0
	)

func _outrage_bomb_particle_color(base_color: Color) -> Color:
	var ember := base_color.lerp(Color(1.0, 0.88, 0.22, 1.0), 0.32)
	var smoky := ember.darkened(0.52)
	return Color(smoky.r, smoky.g, smoky.b, 0.95)
