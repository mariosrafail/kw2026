...
var send_spawn_erebus_shield_cb: Callable = Callable()

var _outrage_bomb_skill
var _erebus_immunity_skill
var _erebus_shield_skill
...
func configure(state_refs: Dictionary, callbacks: Dictionary) -> void:
	...
	send_spawn_erebus_shield_cb = callbacks.get("send_spawn_erebus_shield", Callable()) as Callable
	...
	if _erebus_shield_skill == null:
		_erebus_shield_skill = EREBUS_SHIELD_SKILL_SCRIPT.new()
	...
	if _erebus_shield_skill != null and _erebus_shield_skill.has_method("configure"):
		_erebus_shield_skill.call(
			"configure",
			{
				"players": players,
				"multiplayer": multiplayer
			},
			{
				...
				"get_lobby_members": get_lobby_members_cb,
				"character_id_for_peer": Callable(self, "_warrior_id_for_peer"),
				"send_spawn_erebus_shield": send_spawn_erebus_shield_cb
			}
		)
...
func server_cast_skill(slot: int, caster_peer_id: int, target_world: Vector2) -> void:
	...
	if warrior_id == WARRIOR_EREBUS:
		if slot == 1:
			...
			if _erebus_shield_skill != null and _erebus_shield_skill.has_method("server_cast_skill2"):
				_erebus_shield_skill.call("server_cast_skill2", caster_peer_id, target_world)
		return
...
func client_spawn_erebus_shield(peer_id: int, duration_sec: float) -> void:
	if _erebus_shield_skill != null and _erebus_shield_skill.has_method("client_spawn_shield"):
		_erebus_shield_skill.call("client_spawn_shield", peer_id, duration_sec)
