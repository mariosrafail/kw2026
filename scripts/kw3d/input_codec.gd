...
static func decode(bytes: PackedByteArray) -> Array:
	...
	for i in range(bytes.size()/STRIDE):
		...
		if flags>31 or bytes[p+33]!=0 or bytes[p+34]!=0 or bytes[p+35]!=0:return []
		result.append({"seq":bytes.decode_u32(p),"ct":bytes.decode_u32(p+4),"js":bytes.decode_u32(p+8),"gs":bytes.decode_u32(p+12),"move":Vector2(bytes.decode_float(p+16),bytes.decode_float(p+20)),"yaw":bytes.decode_float(p+24),"pitch":bytes.decode_float(p+28),"fire":bool(flags&1),"aim":bool(flags&2),"sprint":bool(flags&4),"side":1.0 if flags&8 else -1.0,"reload":bool(flags&16)})
	return result
