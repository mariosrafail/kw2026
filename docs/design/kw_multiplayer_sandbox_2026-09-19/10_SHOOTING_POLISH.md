...
- Locomotion was rebuilt around the supplied Rayman-style pose cycle instead of planted walking: left foot front/right foot back with torso+head high, brief low/contact compression under the body, then the mirrored right-front/left-back high pose. QA measures about `±2.03 m` mirrored front/back split, only `9/85` contact frames per tested direction, and torso vertical motion around `-0.07 m` to `+0.16 m`, while the gameplay collision body stays grounded and terrain/ramp/sharp-turn tests pass.
